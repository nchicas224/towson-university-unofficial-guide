...
authority and no organization. Combining the two lets the guide give the rule *and* what
...
- **Labeled context** — `format_context()` prefixes each retrieved chunk with
  `[n] (official|informal, source)`, so the model can actually *act on* the official-vs-informal
...
by centrality" — even though the corpus *does* contain the answer.
...
1. **Asymmetric dense retrieval.** The answer-bearing chunks exist in
   ...
   query matches question-shaped text better than terse answers. (Q3 had the *same* root cause —
   its #1 hit was the original *post*, not the answer comments — which is why the HyDE stretch
   feature was added and fixed it.)
2. **Superlative / aggregative questions aren't localized.** "Which is the *most* central"
   requires comparing across dorms; no single chunk states "The Towers are THE most central."
   Q4's "*how many* halls total" is the same — the count "3" is stated in no chunk, so the model
...
hallucinated a nonexistent dorm ("Richmond Hall"), steering retrieval wrong; *grounding* the
...
refusal is the *correct* behavior given the retrieval gap — the grounding prompt is working; the
